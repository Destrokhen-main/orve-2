...
describe('парсинг children', () => {
  ...
  test('HTML', () => {
    ...
    expect(parsedArray).toStrictEqual([{ type: 'HTML', value: '<div>TEST</div>', node: null }]);
  });

  test('div node', () => {
    const array = [Node("div", null, "HELLO WORLD")];

    const parsedArray = parseChildren.call({ __CTX_ID__: true, __CTX_PARENT__: true }, array, null);
