...
import { textNodeCreator, htmlNodeCreate, RefChildCreator, RefFormateChildCreator } from "./helper";

import { mounterNode } from "./index";

type InsertType = NodeOP | NodeChild | NodeHtml 
  ...
  return listNode.map((item: InsertType) => {
    if (item.type === TypeNode.Component) {
      ...
      if (knowItem.tag !== FRAGMENT) {
        const r = mounterNode(root, knowItem);
      }
